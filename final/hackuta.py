# -*- coding: utf-8 -*-
"""
Created on Sun Sep 29 09:00:10 2019
...
conn=sqlite3.connect('SnakesandLadders.db')
c=conn.cursor()
pygame.init()
snakes = {
    98: 79,
...
# =============================================================================
    ...
    print("execute")
    c.execute('SELECT QID FROM Answers')
    id= c.fetchall()
    n= random.choice(id)
    print(n)
    c.execute('SELECT Question FROM Answers where QID is (?)', (n))
    que= c.fetchall()
    print("1." , que)
    
    c.execute('SELECT Option1 FROM Answers where QID is (?)', (n))
    opt1= c.fetchall()
    print("A. " ,opt1)
    
    c.execute('SELECT Option2 FROM Answers where QID is (?)', (n))
    opt2= c.fetchall()
    print("B. " ,opt2)
    
    c.execute('SELECT Option3 FROM Answers where QID is (?)', (n))
    opt3= c.fetchall()
    print("C. ",opt3)
    
    c.execute('SELECT Option4 FROM Answers where QID is (?)', (n))
    opt4= c.fetchall()
    print("D. ",opt4)
    
    c.execute('SELECT Answers FROM Answers where QID is (?)', (n))
    ans=c.fetchall()
    answer = ans[0][0]
